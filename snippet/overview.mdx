...
| `data-customer-id` | | Stripe customer ID (`cus_…`) for auto-init. If omitted, call `window.Gatlio.check(cid)` manually |
...
window.Gatlio.check('cus_xxxxxxxxxxxxxxxx')
```

Call `Gatlio.check()` again any time the signed-in user changes (e.g. account switcher).
